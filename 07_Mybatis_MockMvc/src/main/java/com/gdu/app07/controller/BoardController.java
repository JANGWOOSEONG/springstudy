...
@Controller
@RequestMapping("/board") // /board로 시작하는 모든 맵핑은 내가 받겠다
public class BoardController {
	...
	// 목록
	// getBoardList() 서비스가 반환한 List<BoardDTO>를 /WEB-INF/views/board/list.jsp로 전달! 하는게(forward) 포워드할 데이터는 모델 Model에 저장 /list.do가 하는일
	@GetMapping("/list.do")
	...
	*/
	...
	// addBoard() 서비스 반환한 0 또는 1을 가지고 /board/list.do으로 이동(redirect)한다.
	// addBoard() 서비스 반환한 0 또는 1은 /WEB-INF/views/board/list.jsp에서 확인한다.
	@PostMapping("/add.do")
	...
	// modifyBoard() 서비스 반환한 0 또는 1을 가지고 /board/detail.do으로 이동(redirect)한다.
	// modifyBoard() 서비스 반환한 0 또는 1은 /WEB-INF/views/board/detail.jsp에서 확인한다.
