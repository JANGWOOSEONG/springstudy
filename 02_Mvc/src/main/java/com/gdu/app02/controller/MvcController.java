...
public class MvcController {
	...
	*/
	...
	/*
		...
		CharacterEncodingFilter를 통해서 자동으로 UTF-8로 인코딩 된다.
	...
	/*
		2. @RequestParam으로 요청 파라미터 처리하기
			...
			3) defaultValue : 요청 파라미터가 없을 때(null) 대신 사용할 값
	*/
	
	@GetMapping("/detail.me")
	public String detailMe(@RequestParam(value="name", required = false, defaultValue = "홍길동") String name, // 요청 파라미터의 name이 없으면 "홍길동"을 사용한다.
			        	   @RequestParam(value="age", required = false, defaultValue = "0") int age, // 요청 파라미터의 age가 없으면 "0"을 사용한다.
	                       Model model) {
