...
public class SecondServiceImpl implements ISecondService {
	...
	public BmiVO execute1(HttpServletRequest request, HttpServletResponse response) {
		...
		return new BmiVO(weight, height, bmi, obesity); // $.ajax의 success로 넘기는 메시지
	...
	*/
	...
	public ResponseEntity<BmiVO> execute1(HttpServletRequest request) {
		...
		return new ResponseEntity<BmiVO>(new BmiVO(weight, height, bmi, obesity), HttpStatus.OK); //200
		
	}catch(Exception e) {
		
		BmiVO bmiVO = null;
		return new ResponseEntity<BmiVO>(bmiVO, HttpStatus.INTERNAL_SERVER_ERROR); //500이므로 $.ajax의 error에서 처리됨
	}
